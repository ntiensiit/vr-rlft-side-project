...
def test_load_generated_grasps_plain_array(tmp_path: Path) -> None:
    ...
    if not (loaded.shape == (3, 4, 4)):
        raise AssertionError
    if not (np.allclose(loaded, grasps)):
        raise AssertionError
...
def test_run_single_object_grasp_inference_from_observation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    np.save(obs_path, rng.standard_normal((64, 3)).astype(np.float32))
    ...
    monkeypatch.setattr(
        ...
        lambda _checkpoint, _feature_dim, _num_steps, _device, _seed: "dummy_gen",
    )
    monkeypatch.setattr(
        "grasping_ai.inference.grasp_inference_runtime.generate_candidate_grasps",
        lambda _gen, _pc, _n: dummy_grasps,
    )
    ...
    if not (np.allclose(res, dummy_grasps)):
        raise AssertionError
...
def test_run_single_object_grasp_inference_diffusion_and_flow(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def mock_diffusion(_checkpoint: object, _feature_dim: int, _num_steps: int, _device: str, _seed: int) -> str:
        called_methods.append("diffusion")
        return "diffusion_generator"

    def mock_flow(_checkpoint: object, _feature_dim: int, _num_steps: int, _device: str, _seed: int) -> str:
    ...
    monkeypatch.setattr(
        "grasping_ai.inference.grasp_inference_runtime.generate_candidate_grasps",
        lambda _gen, _pc, n: _sample_grasps(n),
    )
    ...
    if not (called_methods == ["diffusion", "flow"]):
        raise AssertionError
...
def test_run_single_object_grasp_inference_from_ycb_mesh(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _root, _obj_id: fake_mesh_path,
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "grasping_ai.inference.grasp_inference_runtime.build_diffusion_grasp_generator",
        lambda _checkpoint, _feature_dim, _num_steps, _device, _seed: "gen",
    )
    monkeypatch.setattr(
        "grasping_ai.inference.grasp_inference_runtime.generate_candidate_grasps",
        lambda _gen, _pc, n: _sample_grasps(n),
...
def test_generate_candidate_grasps_batch() -> None:
    ...
    def dummy_gen(_point_cloud: np.ndarray, count: int) -> np.ndarray:
        return _sample_grasps(count)

    results = [generate_candidate_grasps(dummy_gen, pc, 2) for pc in [pc1, pc2]]
    if not (len(results) == EXPECTED_RESULT_COUNT):
        raise AssertionError
    if not (results[0].shape == (2, 4, 4)):
        raise AssertionError
...
def test_write_generated_grasps_exception_and_array_writer(
    ...
    def bad_save(*_args: object, **_kwargs: object) -> None:
