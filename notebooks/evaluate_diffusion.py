...
# %% [markdown]
# # Diffusion Grasp Evaluation
#
# **Objective.** Evaluate generative diffusion grasp poses on analytical and simulation metrics.
#
...
cfg = load_notebook_config(CONFIG_DIR, "config", overrides=["evaluation=diffusion", "training=diffusion"])
...
print_runtime_banner(
    ...
    config_names={"diffusion": DIFFUSION_CONFIG_NAME},
    root=root,
    device=device,
    seed=seed,
)
...
print("diffusion", {"path": str(checkpoint_path), "exists": exists})
if not exists:
...
diffusion_filter_collisions = yaml_config.value( "evaluation", "filter_collisions", value_type=bool, default=False)
...
setup_logging(module_name="evaluate_diffusion")
...
comparison = {
    "object_id": object_id,
    "diffusion_aggregated": diffusion_aggregated,
